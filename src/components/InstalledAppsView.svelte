	...
	let iconProcessing = false;
		...
		// GitHub issues
		let issuesDone = $state(null);
		const GITHUB_ISSUES_URL =
			"https://github.com/mehdiraized/npmax/issues/new";
	...
	async function processIconBatch() {
		...
		function openCatalogIssue() {
			...
			const body = [
				...
				"",
				"## Task",
				"Please add these app(s) to `src/data/appCatalog.js` so npMax can track newer versions automatically.",
				"",
				"---",
		...
		function reportApp(app) {
			const body = [
				...
				"",
				"## Request",
				"Please add this app to `src/data/appCatalog.js` so npMax can detect updates for it.",
				"",
				"## Notes",
				"- Update URL, official website, and release source if known:",
				"- Anything else that helps identify the app:",
				"",
				"---",
				"*Opened from npMax Installed Apps view*",
			].join("\n");
...
<div class="view">
	...
	{#if issuesDone}
		<div class="result-banner">
			<span>
				{#if issuesDone.type === "catalog"}
					GitHub issue page opened with {issuesDone.count} unmatched app{issuesDone.count === 1
						? ""
						: "s"} pre-filled. Just review and click submit.
				{/if}
			</span>
...
<style lang="scss">
	...
	.tag--action {
		...
		&:hover {
			background: rgba(91, 156, 246, 0.24);
