<script>
	...
	import {
		fetchFlutterPackagesInfo,
		getFlutterPackageDetails,
	} from "../api/flutter.js";
	import {
		checkFlutterLockFile,
		getProjectFlutterManifest,
		runFlutterPubGet,
		updateFlutterPackageVersion,
	} from "../utils/shell.js";
	import { normalizeFlutterVersion, parsePubspec } from "../utils/flutter.js";

	let { project, rawJson, onRefresh = () => {} } = $props();

	let manifest = $derived.by(() => parsePubspec(rawJson));
	...
	const isFetchable = (dependency) => dependency.source !== "pubspec-manual";
	...
	$effect(() => {
		...
		for (const dependency of dependencies) {
			initial[dependency.id] = isFetchable(dependency)
				? { status: "loading" }
				: { status: "unsupported" };
		}
		infoMap = initial;
		...
		fetchFlutterPackagesInfo(fetchable, (id, data) => {
			infoMap = {
				...infoMap,
				[id]: data
	...
	$effect(() => {
		const _ = rawJson;
		if (project?.path) lockStatus = checkFlutterLockFile(project.path);
	});

	function getStatus(dependency) {
		const info = infoMap[dependency.id];
		if (!info || info.status === "loading") return "loading";
		if (info.status === "unsupported") return "manual";
		if (info.status === "error") return "error";
		const latest = normalizeFlutterVersion(info.version);
		const current = normalizeFlutterVersion(dependency.version);
	...
	async function openDependencyDetails(dependency) {
		...
		try {
			packageDetails = await getFlutterPackageDetails(dependency);
		} catch (err) {
	...
	async function handleUpdate(dependency) {
		...
		try {
			const updated = await updateFlutterPackageVersion(
				project.path,
				dependency,
				latestVersion,
			);
			...
			const refreshed = await getProjectFlutterManifest(project.path);
			onRefresh(refreshed);
		} catch (err) {
	...
	async function handleInstall() {
		installing = true;
		try {
			await runFlutterPubGet(project.path);
			lockStatus = checkFlutterLockFile(project.path);
			toast.success("flutter pub get complete", { position: "bottom-center" });
		} catch (err) {
			toast.error("flutter pub get failed", {
				description: err.message,
				position: "bottom-center",
			});
		} finally {
...
<div class="editor">
	<div class="editor__meta">
		<div class="editor__metaLeft">
			<span class="meta__filename">pubspec.yaml</span>
			<span class="meta__version">Flutter / Dart</span>
		</div>
	...
	<div class="editor__body">
		<div class="code">
			<div class="line"><span class="tok-brace">pubspec.yaml</span></div>
			{#if dependencies.length === 0}
				<div class="line line--empty">No Flutter packages found.</div>
			{:else}
				{#each dependencies as dependency}
					<div class="line pkg-line">
						...
						<span class="pkg-status">
							...
							{/if}
							<a class="link-icon" href={`https://pub.dev/packages/${dependency.name}`} title="pub.dev">
