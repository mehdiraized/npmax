...
import { openUrl, toolsVersions } from "../lib/host";
import { startWindowDrag } from "../lib/drag";
import { PKG_ICONS } from "../components/PkgIcons";
...
function cleanToolVersion(key: string, raw: string) {
  ...
  if (!first || /this project|not found|command not found/i.test(first)) {
    return "";
  }
    ...
    <aside className="nav">
      <div className="nav__scroll" style={{ overflow: "auto", flex: 1 }}>
        <section className="nav__section">
          <div className="nav__topActions" onMouseDown={startWindowDrag}>
            <button
              type="button"
        ...
        <section className="nav__section">
          <button
            type="button"
            className={`nav__item ${active === "installed-apps" ? "nav__item--active" : ""}`}
          ...
          >
            ...
            <span className="nav__itemBadge">System</span>
          </button>
          <button
            type="button"
